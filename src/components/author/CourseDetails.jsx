    ...
    useEffect(() => {
        const getAllVideosWithModules = async () => {
            ...
            try {
                ...
                const response = await axios.get("http://backend:8080/api/video/getAllVideos/" + params.cid, {
        ...
        const getReviews = async () => {
            try {
                 let token = localStorage.getItem('token');
                const response = await axios.get("http://backend:8080/api/review/getReviewsByCourse/" + params.cid,{
                    headers: { 'Authorization': 'Bearer ' + token }
                })
                console.log(response);
